---
title: "Fetch and Update Jira Data in an Excel Spreadsheet"
tags:
...
In this tutorial, we will go through a Python script that uses the Jira API to retrieve data and update an Excel spreadsheet. This script fetches Jira issues based on filter IDs and inserts them into different sheets in an Excel workbook. Additionally, it updates timestamp and refreshes pivot tables in the workbook.
...
Refreshes pivot tables in the "Pivots" worksheet.

- `wb`: The Excel workbook to refresh.
...
The script's main execution is within the `if __name__ == '__main__':` block. This section fetches Jira data, writes it to the appropriate worksheet, and updates the timestamp and pivot tables in the Excel workbook.
...
This script will fetch data from Jira and update your Excel workbook with the relevant information in separate worksheets, refresh pivot tables, and update the timestamp.
...
JIRA_API_URL = "https://securian.atlassian.net/rest/api/latest/search?jql=filter=" 
API_KEY = '' 
USER_NAME = ''
...
def insert_stories(ws, data):
    ...
    for issue in data: 
        ...
        try:
            ...
            parent_summary = ''
        story_point = issue['fields']['customfield_10016'] if issue['fields']['customfield_10016'] else ''
        project_name = issue['fields']['project']['name']
        sprint = issue['fields']['customfield_10020'][0]['name'] if issue['fields']['customfield_10020'] else ''
...
def insert_bugs(ws, data):
    ws.delete_rows(1,ws.max_row)
    project_priority = {
        "Viva Modernization Quality": "customfield_12301",
        "Viva Modernization Pod 0": "customfield_12350",
        "Viva Modernization Pod 1": "customfield_12351",
        "Viva Modernization Pod 2": "customfield_12352",
        "IMT_ET_Affinity Solutions_REST": "customfield_11549",
        "IMT_ET_Affinity Solutions_NBC": "customfield_12356",
        "VIVA Batch and Integrations": "customfield_12347"
        }
    ...
    for issue in data: 
        ...
        project_name = issue['fields']['project']['name']
        try:
            priority = issue['fields'][project_priority[project_name]]['value']
        except:
            priority = "None"
...
def refresh_pivots(wb):
    ...
    pivots = ws._pivots
    for pivot in pivots:
        pivot.cache.refreshOnHload = True
...
if __name__ == '__main__': 
    file_path = "C:\\Users\q4n5r\\OneDrive - Securian Financial\\Documents\Viva QE Metrics.xlsx"
