---
title: "Known Issues and Workarounds for Apple's Same-Origin Policy in Safari when Dealing with Iframes in Automated Testing"
tags:
  - title: "Known Issues and Workarounds for Apple's Same-Origin Policy in Safari when Dealing with Iframes in Automated Testing"
...
seoDescription: "Known Issues and Workarounds for Apple's Same-Origin Policy in Safari when Dealing with Iframes in Automated Testing"
covers:
...
We will illustrate how to use Playwright to interact with a webpage containing a iframes. 
...
```
---
Here's a demo of the test run. It navigates to the specific page, identifies the element inside the Neste iFrame, and interacts with it to load the "Inside View" of the car.
...
In this example, we use Playwright to locate the iframe, switch to it, interact with the content, and then switch back to the parent context. Playwright's `FrameLocator` makes it easier to handle iframes, even in browsers like Safari with strict SOP.
